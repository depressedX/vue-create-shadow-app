export class VueElement implements ComponentCustomElementInterface {
  _isVueCE = true
  /**
   * @internal
   */
   ...
   * @internal
   */
  _setProp(key: string, val: any, shouldReflect = true, shouldUpdate = false): void {
  ...
  private _applyStyles(styles: string[] | undefined, owner?: ConcreteComponent) {
    if (!styles) return
    if (owner) {
  ...
  private _parseSlots() {
    ...
    while ((n = this.firstChild)) {
      const slotName = (n.nodeType === 1 && (n as Element).getAttribute('slot')) || 'default'
